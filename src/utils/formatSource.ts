const addPrefix = (url: string) => `https://${url}`;

export function fSlug(string: string) {
  string = string.trim();
  ...
  if (
    /^mangadex\.org\/title\/[a-f0-9]{8}-[a-f0-9]{4}-4[a-f0-9]{3}-[89aAbB][a-f0-9]{3}-[a-f0-9]{12}(\/|$)/.test(
      string
    )
  )
    return `/mangadex/${string.split('/')[2]}`;
  ...
  if (/^blogtruyen\.vn\/[0-9]{1,6}(\/|$)/.test(string))
    return `/blogtruyen/${string.split('/')[1]}`;
  if (/^reddit\.com\/r\/\w+\/comments\/\w{6}(\/|$)/.test(string))
    return `/reddit/${string.split('/')[4]}`;
  if (/^reddit\.com\/comments\/\w{6}(\/|$)/.test(string)) return `/reddit/${string.split('/')[2]}`;
  if (/^drive\.google\.com\/file\/d\/.{33}(\/|$)/.test(string))
    return `/googledrive/${string.split('/')[3]}`;
  return '';
}

export function fSource(source: string, slug: string) {

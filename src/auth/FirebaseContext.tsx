...
export function AuthProvider({ children }: AuthProviderProps) {
  ...
  const login = useCallback(async (email: string, password: string) => {
    await signInWithEmailAndPassword(AUTH, email, password).catch((error: AuthError) => {
      throw new Error('Login failed! Try another login methods');
    });
  }, []);
    ...
    await signInWithPopup(AUTH, GITHUB_PROVIDER)
      ...
      .catch((error: AuthError) => {
        if (error.code == 'auth/account-exists-with-different-credential') {
          ...
          const dummy = error.customData;
          console.log(dummy);
          return fetchSignInMethodsForEmail(AUTH, existingEmail!)
            .then((methods) => {
              if (methods.indexOf(GoogleAuthProvider.PROVIDER_ID) !== -1) {
                const googleProvider = new GoogleAuthProvider();
                googleProvider.setCustomParameters({ login_hint: existingEmail! });
                return signInWithPopup(AUTH, GOOGLE_PROVIDER).then((result) => {
                  return result.user;
                });
              }
            })
